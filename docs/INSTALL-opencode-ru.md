...
> В opencode 2.x MCP-процессами владеет фоновый сервис (`opencode2.exe serve --service`),
> и он переживает закрытие окна и редактора. Если перезапуск не помог, переключи
> `"enabled": false` → `true` в `opencode.json`: сервис перечитает конфиг и поднимет
> сервер заново. И не верь одному только `opencode mcp list` — команда создаёт свой
> инстанс, поэтому может показать `✓ connected`, хотя в твоей сессии инструментов нет.
...
| Инструменты MCP пропали из сессии после замены exe или убийства процесса | opencode 2.x держит фоновый сервис (`opencode2.exe serve --service`), который владеет MCP-процессами; мёртвый он **не** перезапускает, а перезапуск окна сервис не перезапускает | переключи `"enabled": false` → `true` в `opencode.json` и подожди несколько секунд. Не верь `opencode mcp list`: команда поднимает свой инстанс и показывает `✓ connected`, даже когда сервер твоей сессии мёртв |

Пересборка: `git pull` в клоне форка → `deno compile -A --output Rvt_Docs_MCP-windows.exe main.ts`
→ снять запущенный MCP-процесс (Windows блокирует работающий exe: перезапись падает с
`Access is denied`) → заменить exe в `tools/` → переключить `"enabled"` в `opencode.json`,
чтобы сервис поднял его заново.
...
| Инструментов нет в сессии, хотя `mcp list` зелёный | сессия запущена до создания конфига, либо фоновый сервис держит мёртвый MCP-процесс | **перезапусти opencode** (закрой и открой заново); если не помогло — переключи `"enabled": false` → `true` в `opencode.json`: процесс `serve --service` переживает закрытие окна |
